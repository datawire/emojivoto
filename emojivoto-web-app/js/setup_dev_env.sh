...
install_upgrade_telepresence() {
    ...
    if [ $install_telepresence = true ]; then
        sudo curl -fL https://app.getambassador.io/download/tel2/${OS}/${ARCH}/2.20.3-blackbird.3-dev.0/telepresence -o /usr/local/bin/telepresence
...
connect_local_dev_env_to_remote() {
    ...
    telepresence quit -s
    telepresence connect --docker --context default --namespace ${EMOJIVOTO_NS}
